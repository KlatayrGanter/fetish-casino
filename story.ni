...
Rebecca is a woman. Understand "woman in blue" or "woman" as Rebecca.
...
The invitation is a thing carried by the player. The description is "The small card you had been given.".

Understand "card" as the invitation.
Understand "entry" as the invitation.
...
Understand "think about [any thing]" or "think about [any known thing]" as thinking about.
...
Part 1.1.1b - Fetish and chips room

The fetish-and-chips is a room. "A man in blue shorts is standing behind a wooden counter, with some foto albums on it. A door behind the counter leads to a garderobe, but you cannot reach it from this side.".

The fetish-and-chips is in the casino. The man in blue shorts is in the fetish-and-chips.

The fetish-and-chips counter is a thing in the fetish-and-chips. It is scenery. The printed name is "the fetish-and-chips counter". The description is "A wooden counter with some picture books.".

Understand "counter" or "desk" as the fetish-and-chips counter.


The picture book is a container on the fetish-and-chips counter. It is scenery. It is openable. The initial appearance is "Large picture books are strewn out on the counter. Monica immediately starts flipping through.". The printed name is "a picture book". The description is "A large picture book lies open on the counter.".

Understand "book", "foto album" or "catalogue" as the picture book.

The garderobe is a room. The garderobe is west of the fetish-and-chips.

The classic style mirror is a thing. The printed name is "A classic style mirror". The description is "A long mirror with a black frame, long enough to show you awhole. At some places the silver is worn a bit.".

Understand "reflection" or "mirror" as the classic style mirror.

The dressing room is a room. The description of the dressing room is "A full-length mirror is here.".
The classic style mirror is scenery in the dressing room.

A dressing room curtain is a door. It is north of the fetish-and-chips and south of the dressing room.
...
Chapter 1.2.1e - random thoughts
...
Chapter 1.2.1f - a chain of thoughts
...
Instead of doing something other than going west when the player is wearing a sundress and the player is in the entrance and the entrance is mapped east of the fetish-and-chips:
	If Rebecca has the invitation:
		remove the invitation from play;
		Monica drags you westwards in two turns from now;
	say "[one of][m]Come on! Let's go![r] Monica shouts[r] Monica shouts[or][m]What are you waiting for?[r] Monica grabs your hand and start pulling you towards the fetish-and-chips.[or][bracket]Ok, Monica, have it your way..[close bracket][line break][stopping]";
...
Instead of giving the invitation to Rebecca:
	say "The woman, smiles as she accepts it.";
	...
	wait for any key;
	say "First of all, don't feel self-conscious about your clothing. Most people come dressed as you are. In the corner there, you will find a fetish-and-chips.";
	Now a fetish-and-chips is mapped west of the entrance;
	Now The entrance is mapped east of the fetish-and-chips;
	Now the decency threshold of the fetish-and-chips is formal;
	wait for any key;
	...
	wait for any key;
	say "If you find yourself running short on chips, you can return to the fetish-and-chips for more extreme attire. Once you start, it is much like a regular casino.";
	wait for any key;
...
Instead of showing the invitation to Rebecca:
	try giving the invitation to Rebecca.

At the time when Monica drags you westwards:
	Now the player is in the fetish-and-chips;
	Now Monica is in the fetish-and-chips;
	Now Cindy is in the fetish-and-chips;
	Now Rebecca is in the game room;

Chapter 1.2.2 - thing interactions

Chapter 1.2.2a - classic style mirror

Before looking under the classic style mirror:
	say "This mirror starts at the floor.".

Instead of searching the classic style mirror:
	if the player wears no glasses:
		say "You squint your eyes a little to accomodate. [no line break]";
	say "You see yourself in the reflection.[paragraph break][description of Gina]";

Instead of attacking the classic style mirror:
	say "And face the wrath of the casino?".

Instead of touching the classic style mirror:
	say "It feels cold to the touch.".

Chapter 1.2.2b - fetish-and-chips counter

Instead of searching the fetish-and-chips counter:
	say "There are catalogues for fetish wear on top of it.".

Instead of climbing the fetish-and-chips counter:
	say "You're quite sure that's not allowed.".

Instead of consulting the fetish-and-chips counter about:
	say "(The picture book)";
	try searching the picture book;

Instead of going to the garderobe:
	say "The counter is blocking your way.";

Chapter 1.2.2c - the picture book

Instead of taking the picture book:
	say "[r]You[apostrophe]re not allowed to take it with you.[r]".

Instead of searching the picture book:
	say "The pictures show men and women in a variety of fetish clothing. The book you choose is all women, with the first pictures showing simple latex skirts and loose leather tops.".

Instead of consulting the picture book about:
	say "When Monica, on impulse flips to the last page, the three girls gasp at the sight. The woman displayed is wearing a form fitting latex body suit. A corset strapped around her waist, reducing her to a seemingly impossible 20 inches.".

Instead of opening the picture book:
	try searching the picture book;

Instead of switching on the picture book:
	say "[a]So, have you made your decision yet?[r]".

Instead of turning the picture book:
	say "As you turn the pages, it seems the clothing becomes more extreme.".

Instead of buying the picture book:
	say "[a]So, you made a decision?[r]".
